...
| 邮件 | 本地 mail-sink（SMTP 收信 + **网页收件箱** http://localhost:8025），邮件自动解码、登录链接一键可点，全程离线可演示 |
...
端口：前台/后台 `2368`；本地邮件 `mail-sink`——SMTP 1025（容器内部）接收邮件，**网页收件箱 http://localhost:8025**（自动解码每封邮件，内含可点击的"登录链接"与验证码；原始邮件也落盘 `runtime/mail/*.eml`）。
